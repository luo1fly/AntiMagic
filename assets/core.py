...
class Asset(object):
    def __init__(self, request):
        """
        :param request: request object contains custom requests
        :param mandatory_fields: 强制检测必须要有的字段
        :param response:
        """
        ...
        try:
            if not only_check_sn:
                ...
                setattr(
                    self,
                    'asset_obj',
                    models.Asset.objects.get(sn=data['sn']),
                )
            ...
            setattr(self, 'waiting_approval', True)
            return False
        else:
            return True
    ...
    def get_asset_id_by_sn(self):
        ...
        if data:
            ...
            self.response_msg('error','AssetDataInvalid', "The reported asset data is invalid or not provided")
            response = self.response
        return response

    def save_new_asset_to_approval_zone(self):
        """
        When find out it is a new asset,
        will save the data into approval zone to waiting for IT admin's approvals
        :return:
        """
        asset_sn = self.clean_data.get('sn')
        asset_already_in_approval_zone = \
            models.NewAssetApprovalZone.objects.get_or_create(
                ...
                vendor=self.clean_data.get('manufactory'),
    ...
    def data_inject(self):
        ...
        if self.__is_new_asset():
            ...
            # asset already already exist , just update it
    ...
    def _update_server(self):
        ...
        self.__update_asset_component(
            ...
            fk='nic_set',
            update_fields=['name', 'sn', 'model', 'mac', 'ip', 'net_mask', 'bonding'],
            identify_field='mac',
        )
        # disk
        self.__update_asset_component(
            ...
            fk='disk_set',
            update_fields=['slot', 'sn', 'model', 'vendor', 'capacity', 'iface_type'],
            identify_field='slot',
        )
        # ram
        self.__update_asset_component(
    ...
    def __create_server_info(self, ignore_errs=False):
        try:
            ...
            if not len(self.response['error']) or ignore_errs:
                # no processing when there's no error happen
                data_set = dict(
        ...
        if disk_info:
            for disk_item in disk_info:
                try:
                    ...
                    self.__verify_field(disk_item, 'interface_type', str)
                    ...
                    if not len(self.response['error']):
                        ...
                        data_set = dict(
                            ...
                            model=disk_item.get('model'),
                            interface_type=disk_item.get('interface_type'),
                            vendor=disk_item.get('manufactory'),
                        )
    ...
    def __update_asset_component(self, data_source, fk, update_fields, identify_field=None):
        """
        :param data_source: the data source of this component from reporting data
        :param fk: which key to use to find the connection between main Asset obj and each asset component
        :param update_fields: what fields in DB will be compared and updated
        :param identify_field: use this field to identify each component of an Asset ,
            if set to None,means only use asset id to identify
        :return:
        """
        ...
        try:
            ...
            if hasattr(component_obj, 'select_related'):
                # this component is reverse m2m relation with Asset model
                        ...
                        for source_data_item  in data_source:
                            ...
                            if key_field_data_from_source_data:
                                if key_field_data == key_field_data_from_source_data:
                                    # find the matched source data for this component,then should compare each field in this component to see if there's any changes since last update
                                    self.__compare_component(model_obj=obj,fields_from_db=update_fields,data_source=source_data_item)
                                    break
                                    # must break ast last ,then if the loop is finished ,
                                    # logic will goes for ..else part,then you will know that
                                    # no source data is matched for by using this key_field_data, that means ,
                                    # this item is lacked from source data,
                                    # it makes sense when the hardware info got changed.
                                    # e.g: one of the RAM is broken, sb takes it away,
                                    # then this data will not be reported in reporting data
                            else:
                                # key field data from source data cannot be none
                                self.response_msg(
                                    'warning',
                                    'AssetUpdateWarning',
    ...
    def __filter_add_or_deleted_components(self, model_obj_name, data_from_db, data_source, identify_field):
        ...
        # save all the idenified keys from client data,e.g: [macaddress1,macaddress2]
        if type(data_source) is list:
            ...
            for key,data in data_source.items():
                if data.get(identify_field):
                    ...
                    # workround for some component uses key as identified field e.g: ram
        ...
        if type(all_components) is list:
            ...
            for k, data in all_components.items():
                ...
                if data.get(identify_field):
                    ...
                    # if the identified field cannot be found from data set,then try to compare the dict key
                    if k in add_list:
                        data[identify_field] = k
                        # add this key into dict , because this dict will be used to create new component item in DB
                        will_be_creating_list.append(data)
    ...
    def __compare_component(self, model_obj, fields_from_db, data_source):
        """
        :param model_obj: 具体一条网卡
        :param fields_from_db: 要对比的字段
        :param data_source: 客户端具体对应网卡的数据
        :return:
        """
        ...
        for field in fields_from_db:
            ...
            if val_from_data_source:
                # if type(val_from_db) is unicode:
                #     val_from_data_source = unicode(val_from_data_source)
                # elif type(val_from_db) in (int, long):
                if isinstance(val_from_db, int):
                    val_from_data_source = int(val_from_data_source)
                elif isinstance(val_from_db, float):
                    val_from_data_source = float(val_from_data_source)
                if val_from_db == val_from_data_source:
                    # this field haven't changed since last update
                    pass
                    # print '\033[32;1m val_from_db[%s]  == val_from_data_source[%s]\033[0m' %(val_from_db,val_from_data_source)
                else:
                    print(
                        '\033[34;1m val_from_db[%s] != val_from_data_source[%s]\033[0m' % (val_from_db, val_from_data_source),
                        type(val_from_db),
                        type(val_from_data_source),
                    )
                    db_field = model_obj._meta.get_field(field)
                    # 获取数据库记录某个字段对象而不字段值（getattr(obj,field_name)）
                    db_field.save_form_data(model_obj, val_from_data_source)
                    # 查找并修改数据库某个字段值，必须先获取到字段对象，然后修改，最后保存记录对象
