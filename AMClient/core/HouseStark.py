...
class ArgvHandler(object):
    ...
    def __submit_data(action_type, data, method):
        if action_type in settings.Params['urls']:
            ...
            url = ArgvHandler.__attach_token(url)
            ...
            if method == "get":
                ...
                try:
                    ...
                    req_data = urlopen(url_with_args, timeout=settings.Params['request_timeout'])
                    callback = req_data.read().decode('utf8')
                    print("-->server response:", callback)
                    return callback
                except URLError as e:
                    sys.exit("\033[31;1m%s\033[0m" % e.args[0])
            elif method == "post":
                try:
                    data_encode = urlencode(data)
                    res_data = urlopen(url, data=data_encode.encode('utf8'), timeout=settings.Params['request_timeout'])
                    # urlencode是序列化成字符串，encode是转换成字节形式
            ...
            raise KeyError('action_type:%s not found' % action_type)
    ...
    def report_asset(self):
        ...
        if asset_id:
            # had reported to server before
            asset_data["asset_id"] = asset_id
            post_url = "asset_report"
        else:
            # first time report to server
